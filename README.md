# [Postgres 16.6](https://github.com/livingdocsIO/dockerfile-postgres) [![](https://img.shields.io/badge/docker-livingdocs%2Fpostgres-blue)](https://hub.docker.com/r/livingdocs/postgres)
...
docker run -d  --name postgres -p 5432:5432 -v postgres:/var/lib/postgresql -e POSTGRES_PASSWORD=somepassword livingdocs/postgres:16.6
```
...
docker run --rm -v postgres:/var/lib/postgresql livingdocs/postgres:16.6-upgrade

# After it succeeds, you can run the new image and mount the existing volume
docker run -d --name postgres -p 5432:5432 -v postgres:/var/lib/postgresql livingdocs/postgres:16.6
```

## To build this image manually

```bash
docker build -t livingdocs/postgres:16.6 .
```

With buildx on docker
```bash
# To build and push the multi-arch manifest to docker hub
docker buildx build --platform linux/amd64,linux/arm64 -t livingdocs/postgres:16.6 --push .

docker buildx build --platform linux/amd64,linux/arm64 -t livingdocs/postgres:16.6-upgrade --push  -f Dockerfile.upgrade .
```

With nerdctl on lima/containerd
```bash
nerdctl build --platform=amd64,arm64 -t livingdocs/postgres:16.6 .
nerdctl build --platform=amd64,arm64 -t livingdocs/postgres:16.6-upgrade -f Dockerfile.upgrade .

lima nerdctl push --all-platforms livingdocs/postgres:16.6
lima nerdctl push --all-platforms livingdocs/postgres:16.6-upgrade
```
...
docker run -d -p 5433:5432 --name postgres-1 livingdocs/postgres:16.6
docker run -d -p 5434:5432 --name postgres-2 livingdocs/postgres:16.6 standby -d "host=host.docker.internal port=5433 user=postgres target_session_attrs=read-write"
...
docker run -d -p 5433:5432 --name postgres-1 --network=local --network-alias=postgres -e POSTGRES_HOST_AUTH_METHOD=md5 livingdocs/postgres:16.6
...
docker run -d -p 5434:5432 --name postgres-2 --network=local --network-alias=postgres livingdocs/postgres:16.6 standby -d $DB_URL
docker run -d -p 5435:5432 --name postgres-3 --network=local --network-alias=postgres livingdocs/postgres:16.6 standby -d $DB_URL
